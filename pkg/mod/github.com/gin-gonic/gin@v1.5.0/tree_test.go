	...
	for _, request := range requests {
		...
		if value.handlers == nil {
			if !request.nilHandler {
				t.Errorf("handle mismatch for router '%s': Expected non-nil handle", request.path)
			}
		} else if request.nilHandler {
			t.Errorf("handle mismatch for router '%s': Expected nil handle", request.path)
		} else {
			value.handlers[0](nil)
			if fakeHandlerValue != request.route {
				t.Errorf("handle mismatch for router '%s': Wrong handle (%s != %s)", request.path, fakeHandlerValue, request.route)
			}
		}

		if !reflect.DeepEqual(value.params, request.ps) {
			t.Errorf("Params mismatch for router '%s'", request.path)
		}
	}
}
...
func testRoutes(t *testing.T, routes []testRoute) {
	...
	for _, route := range routes {
		...
		if route.conflict {
			if recv == nil {
				t.Errorf("no panic for conflicting router '%s'", route.path)
			}
		} else if recv != nil {
			t.Errorf("unexpected panic for router '%s': %v", route.path, recv)
		}
	}
}
...
func TestTreeDupliatePath(t *testing.T) {
	...
	for _, route := range routes {
		...
		if recv != nil {
			t.Fatalf("panic inserting router '%s': %v", route, recv)
		}
		...
		if recv == nil {
			t.Fatalf("no panic while inserting duplicate router '%s", route)
		}
	}
...
func TestEmptyWildcardName(t *testing.T) {
	...
	for _, route := range routes {
		...
		if recv == nil {
			t.Fatalf("no panic while inserting router with empty wildcard name '%s", route)
		}
	}
}
...
func TestTreeDoubleWildcard(t *testing.T) {
	...
	for _, route := range routes {
		...
		if rs, ok := recv.(string); !ok || !strings.HasPrefix(rs, panicMsg) {
			t.Fatalf(`"Expected panic "%s" for router '%s', got "%v"`, panicMsg, route, recv)
		}
	}
}
...
func TestTreeTrailingSlashRedirect(t *testing.T) {
	...
	for _, route := range tsrRoutes {
		...
		if value.handlers != nil {
			t.Fatalf("non-nil handler for TSR router '%s", route)
		} else if !value.tsr {
			t.Errorf("expected TSR recommendation for router '%s'", route)
		}
	}
	...
	for _, route := range noTsrRoutes {
		value := tree.getValue(route, nil, false)
		if value.handlers != nil {
			t.Fatalf("non-nil handler for No-TSR router '%s", route)
		} else if value.tsr {
			t.Errorf("expected no TSR recommendation for router '%s'", route)
		}
	}
}
...
func TestTreeRootTrailingSlashRedirect(t *testing.T) {
	...
	if recv != nil {
		t.Fatalf("panic inserting test router: %v", recv)
	}
...
func TestTreeFindCaseInsensitivePath(t *testing.T) {
	...
	for _, route := range routes {
		...
		if !found {
			...
			t.Errorf("Wrong result for router '%s': %s", route, string(out))
		}
	}
